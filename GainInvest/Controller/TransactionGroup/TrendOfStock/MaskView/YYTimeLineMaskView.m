...
- (void)drawDashLine
{
    ...
    CGContextAddLineToPoint(ctx, x, self.stockScrollView.frame.origin.y + self.stockScrollView.bounds.size.height - 15 / 2.f);
    ...
    CGContextAddArc(ctx, x, self.stockScrollView.frame.origin.y + self.selectedPoint.y, 3, 0, 2 * M_PI, 0);
    ...
    NSString *dayText = [NSString stringWithFormat:@"%@",self.selectedModel.Date];
    ...
    if (x + textRect.size.width/2.f + 2 > CGRectGetMaxX(self.stockScrollView.frame))
    {
        ...
        CGContextFillRect(ctx, CGRectMake(CGRectGetMaxX(self.stockScrollView.frame) - 4 - textRect.size.width, self.stockScrollView.frame.origin.y + self.stockScrollView.bounds.size.height - 15, textRect.size.width + 4, textRect.size.height + 4));
    ...
    {
        CGContextSetFillColorWithColor(ctx, UIColorFromRGB(0x659EE0, 1).CGColor);
        CGContextFillRect(ctx, CGRectMake(x-textRect.size.width/2.f, self.stockScrollView.frame.origin.y + self.stockScrollView.bounds.size.height - 15, textRect.size.width + 4, textRect.size.height + 4));
    ...
    CGContextFillRect(ctx, CGRectMake(12 - priceRect.size.width - 4, self.stockScrollView.frame.origin.y + self.selectedPoint.y - priceRect.size.height/2.f - 2, priceRect.size.width + 4, priceRect.size.height + 4));
    [priceText drawInRect:CGRectMake(12 - priceRect.size.width - 4 + 2, self.stockScrollView.frame.origin.y + self.selectedPoint.y - priceRect.size.height/2.f, priceRect.size.width, priceRect.size.height) withAttributes:attribute];
    
    //绘制选中增幅
    NSString *text2 = [NSString stringWithFormat:@"%.2f",self.selectedYValue];
