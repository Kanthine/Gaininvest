...
- (NSURLSessionDataTask *)requestForPostUrl:(NSString*)url parameters:(NSDictionary *)parameters CacheType:(AFNetDiskCacheType)cacheType NetworkActivity:(BOOL)isShow completion:(void (^)(BOOL isCacheData,id responseObject,NSError *error))completion
{
    ...
    if ([parameters.allKeys containsObject:@"cur_size"] && cacheType == AFNetDiskCacheTypeUseCacheThenUseLoad)
    {
        if ([parameters[@"cur_size"] isEqualToString:@"1"])
        {
    ...
    if([AFNetAPIClient isLinkNetSuccess] == NO)
    {
        [self loadDiskCacheWithUrl:url parameters:parameters completion:^(id diskResponseObject, NSError *loadError)
         {
             completion(YES,diskResponseObject,[NSError errorWithDomain:@"请确认您的网络链接正常" code:-100 userInfo:nil]);
         }];
...
//          NSLog(@"responseObject ======= %@",responseObject);
          ...
          {
              if ([[responseObject objectForKey:@"result_code"] intValue] == 7)
              {
                  //异地登录的错误情况
                  NSError *error = [NSError errorWithDomain:@"不显示" code:100 userInfo:nil];
                  completion(NO,nil,error);
                  [UserLocalData setRemoteLogin:YES];
              ...
              {
                  if ([UserLocalData isRemoteLogin])
                  {
                      [UserLocalData setRemoteLogin:NO];
                  }
              }
          }
      ...
      } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error)
      {
          ...
          {
              completion(NO,nil,[NSError errorWithDomain:@"请检查您的网络是否正常" code:error.code userInfo:nil]);
          }
          else
          {
              completion(NO,nil,error);
...
- (void)tipAgainLogin
{
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"isLaunch"] boolValue])
    {
        [self performSelector:@selector(tipAgainLogin) withObject:nil afterDelay:1];
        return;
    }

    UINavigationController *nav = [MainTabBarController shareMainController].selectedViewController;
    [AuthorizationManager isRemoteLoginWithViewController:nav.viewControllers.lastObject];
